...
reply :: forall m. (MonadState Req.Req m) => Req.StatusCode -> Req.Headers -> Binary -> m Unit
...
path :: forall m. (MonadState Req.Req m) => m Binary
path = Req.path <$> get

qs :: forall m. (MonadState Req.Req m) => m Binary
qs = Req.path <$> get
